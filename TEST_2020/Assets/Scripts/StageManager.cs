...
public class StageManager : ISystem {
	...
	public List<BlockDB> blockDBs;
	public BlockDB blockDBsUnit;
	...
	void Start()
	{
		...
		string stageIndex = scene.name.Remove(0,5);
		
		for (int i =1;i<=gates.Count;i++)
		{
			string fileName = "BlockDB_" + stageIndex + "_" + i ;
			blockDBsUnit=(BlockDB)Resources.Load(fileName,typeof(BlockDB));
			blockDBs.Add(blockDBsUnit);
		}
	...
	void CreateBlocks()
    {
		BlockDB gateBlocks = blockDBs[gatesIndex];
		for(int i =0; i< gateBlocks.blockData.Count;i++)
        {
			for (int j = 0; j < gateBlocks.blockData[i].posList.Count; j++)
			{
				Vector3 pos = gateBlocks.blockData[i].posList[j];
				BlockBase block = GameManager.ctrl.objectPool.Reuse(gateBlocks.blockData[i].block);
				if (block != null)
				{
				...
				{
					block = Instantiate(gateBlocks.blockData[i].block, pos, Quaternion.identity);
				}
